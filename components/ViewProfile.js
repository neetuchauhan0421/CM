...
import { Platform, StyleSheet, Text, View, Image, Linking, KeyboardAvoidingView, Alert, TouchableOpacity, RefreshControl } from 'react-native';
import { Appbar, Card, Button, Provider as PaperProvider, TextInput, Colors, DefaultTheme, Checkbox, ThemeProvider, Switch } from 'react-native-paper';
...
class ViewProfile extends Component {
   ...
   getUserDetails() {
      ...
      axios.get(serverURI + '/readMember/' + this.props.userLogged)
         .then(response => {
            this.setState({
            ...
            // ---- MEMDATE
            if (response.data.sharingdefaultmemdate === 'y') {
               this.setState({ s_MemDateFlag: true })
            }
            else if (response.data.sharingdefaultmemdate === 'n') {
               this.setState({ s_MemDateFlag: false })
            }

            // ---- LOCATION
            if (response.data.sharingdefaultlocation === 'y') {
               this.setState({ s_Location: true })
            }
            else if (response.data.sharingdefaultlocation === 'n') {
               this.setState({ s_Location: false })
            }

            // ---- PHONE
            if (response.data.sharingdefaultphone === 'y') {
               this.setState({ s_Phone: true })
            }
            else if (response.data.sharingdefaultphone === 'n') {
               this.setState({ s_Phone: false })
            }

            // ---- EMAIL
            if (response.data.sharingdefaultemail === 'y') {
               this.setState({ s_Email: true })
            }
            else if (response.data.sharingdefaultemail === 'n') {
               this.setState({ s_Email: false })
            }

            // ---- SOCIAL
            if (response.data.sharingdefaultsocialacc === 'y') {
               this.setState({ s_Social: true })
            }
            else if (response.data.sharingdefaultsocialacc === 'n') {
               this.setState({ s_Social: false })
            }


            // ---- MEETINGS
            if (response.data.sharingdefaultsetmeetings === 'y') {
               this.setState({ s_setMeetings: true })
            }
            else if (response.data.sharingdefaultsetmeetings === 'n') {
   ...
   render() {
      ...
      return (
         <View style={{ flex: 1, backgroundColor: '#F2F2F2' }}>
            ...
            {/* ?--------------     APPBAR      ---------------- */}
            <Appbar.Header style={{ backgroundColor: '#000000', marginLeft: 2 }}>
               <Appbar.Content
                  title={<Text style={{ color: 'white' }}>Profile</Text>}

               />
               <TouchableOpacity>
                  <Text
                     onPress={() => this.props.navigation.navigate("EditProfile", { onNavigateBack: this.handleOnNavigateBack })}
                     style={{ marginRight: '3%', color: 'white', fontSize: 16 }}>
                     Edit </Text>
               </TouchableOpacity>
            </Appbar.Header>
            ...
            >
               ...
               <View style={styles.container0}>
                  ...
                  <Card style={styles.profileA}>
                     <View style={{ flex: 1, flexDirection: 'row' }}>
                        ...
                        <View style={{ alignItems: 'flex-start', marginLeft: 10 }}>
                           ...
                           <Text style={{ width: 250 }}>{this.state.profileData.membershortbio}</Text>
                        </View>
                     </View>

                     <View style={styles.memberSince}>
                        <Text style={{ marginLeft: 85, marginTop: 6, textTransform:'capitalize' }}>Member Since{': '}{this.state.membershipdate}</Text>
                        <Switch
                  ...
                  <Card style={styles.profileB}>
                     ...
                     {/* <Text>{this.state.profileData.memberabout}</Text> */}
                     {
                        this.state.profileData.memberabout === ''
                           ?
                           <Text style={{ fontSize: 16, color: '#BBBBBB' }}>{' '}Please complete this field...</Text>
                           :
                  ...
                  <Card style={styles.profileC}>
                     ...
                     {/* Location */}
                     <View style={{
                        flexDirection: 'row', flex: 0, height: 30,
                        //marginTop: 15,
                     }}>
                        <Icon2 name="home" color="#8320F4" size={20} />
                        {
                           this.state.profileData.memberlocation === ''
                              ?
                              <Text style={{ margin: 2, color: '#BBBBBB' }}>{' '}Please complete this field...</Text>
                              :
                        ...
                        <Switch
                           ...
                           style={{ position: 'absolute', right: 0, transform: [{ scaleX: .8 }, { scaleY: .8 }] }}
                        />
                     </View>
                     ...
                     <View style={styles.shares}>
                        ...
                        {/* <Text style={styles.shareText}>{'  '}{this.state.profileData.memberphonenumber}</Text> */}
                        {
                           this.state.profileData.memberphonenumber === ''
                              ?
                              <Text style={{ margin: 2, color: '#BBBBBB' }}>{'  '}Please complete this field...</Text>
                              :
                              <Text style={styles.shareText}>
                     ...
                     <View style={styles.shares}>
                        ...
                        {
                           this.state.profileData.memberemail === ''
                              ?
                              <Text style={{ margin: 2, color: '#BBBBBB' }}>{'  '}Please complete this field...</Text>
                              :
                              <Text style={styles.shareText}>
                     ...
                     <View style={styles.shares}>
                        <Icon2 name="hashtag" color="#8320F4" size={18} />
                        {/* <Text style={styles.shareText}>{'  '}{this.state.profileData.membersocialaccount}</Text> */}
                        {
                           this.state.profileData.membersocialaccount === ''
                              ?
                              <Text style={{ margin: 2, color: '#BBBBBB' }}>{'  '}Please complete this field...</Text>
                              :
                              <Text style={styles.shareText}>
                  ...
                  <Card style={styles.profileD}>
                     <View style={styles.sharesProfileD}>
                        <Text style={{ paddingTop: 6 }}>Setup Meetings with you.</Text>
                        <Switch
                           color={'#89CE3A'}
                           onValueChange={(value) => this.setState({ s_setMeetings: value })}
                           value={this.state.s_setMeetings}
                           style={{ position: 'absolute', right: 5, transform: [{ scaleX: .8 }, { scaleY: .8 }] }} />
                     </View>
                  </Card>
